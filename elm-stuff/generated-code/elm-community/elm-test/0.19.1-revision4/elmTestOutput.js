...
var $author$project$Main$calculateScore = function (cards) {
	var scoreFunction = F2(
		function (cardsList, score) {
			scoreFunction:
			while (true) {
				var makeEqualLength = F2(
				...
				if (!cardsList.b) {
					...
					var x = cardsList.a;
					var xs = cardsList.b;
					if (!_Utils_eq(x.face, $author$project$Card$Ace)) {
						...
						score = $temp$score;
						continue scoreFunction;
					} else {
						var $temp$cardsList = xs,
							$temp$score = _Utils_ap(
							A3(
								$elm$core$List$map2,
								$elm$core$Basics$add,
								score,
	...
	var listPossibleScores = $elm$core$List$sort(
		A2(
			scoreFunction,
			cards,
			_List_fromArray(
				[0])));
	var finalScore = ($elm$core$List$length(
		A2(
			$elm$core$List$filter,
			function (x) {
				return x <= 21;
			},
			listPossibleScores)) > 0) ? $elm$core$List$head(
		$elm$core$List$reverse(
			A2(
				$elm$core$List$filter,
				function (x) {
				...
				listPossibleScores))) : $elm$core$List$head(listPossibleScores);
	if (finalScore.$ === 'Just') {
		var value = finalScore.a;
		return value;
	} else {
		return 0;
	}
...
var $author$project$Test$Generated$Main202945477$main = A2(
	$author$project$Test$Runner$Node$run,
	{
		globs: _List_Nil,
		paths: _List_fromArray(
			['E:\\An III sem I\\FP\\Project\\tests\\Tests.elm']),
		processes: 8,
		report: $author$project$Test$Reporter$Reporter$ConsoleReport($author$project$Console$Text$UseColor),
		runs: $elm$core$Maybe$Nothing,
		seed: 268463370824643
	},
...
_Platform_export({'Test':{'Generated':{'Main202945477':{'init':$author$project$Test$Generated$Main202945477$main($elm$json$Json$Decode$int)(0)}}}});}(this));
return this.Elm;
})({});
var pipeFilename = "\\\\.\\pipe\\elm_test-9404-1";
